...
int main(int, char**)
{
    ...
    //Create a world
    World *wld = new World;
    wld->setAmbient(Color(0.2));
    wld->setBackground(Color(0.1, 0.1, 0.4));
    
    wld->setSpecular(Color(1.0, 1.0, 1.0));   //specular lighting color

    LightSource *light = new PointLightSource(wld, Vector3D(1, 1, 5), Color(0.5, 0.8, 0.2));
    wld->addLight(light);


    Material *m = new Material(wld);
    ...
    Material *m2 = new Material(wld);
    m2->color = Color(1.0, 0.0, 0.2);
    m2->ka = 0.2;
    m2->kd = 0.9;
    m2->ks = 0.9;
    m2->n = 164;
    ...
    Object *sphere = new Sphere(Vector3D(-5, 0, -10), 3, m, wld);
    wld->addObject(sphere);
    
    Object *sphere1 = new Sphere(Vector3D(-8, 0, -2), 2.8, m, wld);
    wld->addObject(sphere1);

    Object *triangle = new Triangle(Vector3D(0, 0, -10),Vector3D(5, 0, -10),Vector3D(2.5, 4, -10), m2, wld);
    wld->addObject(triangle); 

    Object *cylinder = new Cylinder(Vector3D(10, -4, -8), 1.4, m, wld);
    wld->addObject(cylinder);

    

    engine = new RenderEngine(wld, camera);
